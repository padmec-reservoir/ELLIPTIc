...
class ProblemBase(object):
    """Defines a type of problem. A problem can expose other set-up methods
    ...
    A problem should be decoupled from the kernel pipeline, in a sense that
    it must not depend on what kernels are present on the pipeline.

    A problem might need to be coupled with the MatrixManager class, or
